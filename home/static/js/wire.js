...
export class CircuitWire {
    ...
    drawWire() {
        ...
        const mouseHorizonLine = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'line',
        );
    ...
    setEndWire(dataId, direction) {
        ...
        startComponent.setConnection(
            this.startDir,
            `${endComponent.num}${this.endDir}`,
        );
        endComponent.setConnection(
            this.endDir,
            `${startComponent.num}${this.startDir}`,
        );
    }
    ...
    deleteWire() {
        ...
        const startComponent = circuitComponents[this.start];
        const endComponent = circuitComponents[this.end];
        startComponent?.removeConnection(
            this.startDir,
            `${this.end}${this.endDir}`,
        );
        endComponent?.removeConnection(
            this.endDir,
