...
const drawWire = (event) => {
    if (startPoint) {
        const endPoint = getSVGCoordinates(
            event.clientX + 5,
            event.clientY + 5,
        );
...
const boardDrag = (event) => {
    ...
    const transformInfo = boardEvent
...
export const boardDragStart = (event) => {
    ...
    wireS = Object.values(circuitWires).filter(
        (circuitWire) => circuitWire.start === dataId,
    );
    wireE = Object.values(circuitWires).filter(
        (circuitWire) => circuitWire.end === dataId,
    );
};
...
export const startWire = (event) => {
    if (
        board.classList.contains('probe_voltage_plus') ||
        board.classList.contains('probe_voltage_minus') ||
        board.classList.contains('probe_current') ||
        board.classList.contains('probe_vout_plus') ||
        board.classList.contains('probe_vout_minus')
    ) {
        return;
    }
