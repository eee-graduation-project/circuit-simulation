...
const firstName = {
    'voltage-source': 'V',
    'voltage-signal-source': 'V',
    'voltage-source-voltage-controlled': 'E',
    'voltage-source-current-controlled': 'G',
    'current-source': 'I',
    'current-signal-source': 'I',
    'current-source-voltage-controlled': 'F',
    'current-source-current-controlled': 'H',
    resistor: 'R',
    inductor: 'L',
    capacitor: 'C',
    ground: 'G',
...
const defaultValue = {
    ...
    'current-source-current-controlled': '',
    resistor: 'Ω',
    inductor: 'H',
    capacitor: 'F',
};
...
export class CircuitComponent {
    ...
    setValue(name, value) {
        if (name === 'value') {
            this.value = value;
            const text = this.svgElement.querySelector(
                '.component__text_value',
            );
    ...
    addName() {
        ...
        elementCnt[this.type.split('-')[0]] += 1;
        // elementCnt[this.type] ++;
        return name;
    }
    ...
    createSVGElement() {
        ...
        svgElement.setAttribute(
            'transform',
            `translate(${this.position.x - 15}, ${this.position.y}) rotate(0) translate(0) scale(1, 1) translate(0)`,
        );
    ...
    handleLineClick() {
        this.wires.forEach((wire) => {
            ...
            wire.addEventListener('click', (event) => {
                setProbe(event, 'probe_current', 'current');
            });
            wire.addEventListener('click', (event) => {
    ...
    appendWire() {
        switch (this.type) {
            case 'ground': {
                const wire = this.createLineElement(
                    { x1: 40, x2: 40, y1: 0, y2: 15 },
                    'T',
                );
                return [wire];
            }
            ...
            case 'current-source-voltage-controlled': {
                const wireI = this.createLineElement(
                    { x1: 0, x2: 20, y1: 5, y2: 5 },
                    'I',
                );
                const wireM = this.createLineElement(
                    { x1: 0, x2: 20, y1: 34, y2: 34 },
                    'M',
                );
                const wireT = this.createLineElement(
                    { x1: 46, x2: 46, y1: -15, y2: 1 },
                    'T',
                );
                const wireB = this.createLineElement(
                    { x1: 46, x2: 46, y1: 39, y2: 55 },
                    'B',
                );
            ...
            case 'current-source-current-controlled': {
                const wireT = this.createLineElement(
                    { x1: 32.5, x2: 32.5, y1: -15, y2: 1 },
                    'T',
                );
                const wireB = this.createLineElement(
                    { x1: 32.5, x2: 32.5, y1: 39, y2: 55 },
                    'B',
                );
                return [wireT, wireB];
            }
            default: {
                const wireL = this.createLineElement(
                    { x1: 0, x2: 19, y1: 20, y2: 20 },
                    'L',
                );
                const wireR = this.createLineElement(
                    { x1: 60, x2: 80, y1: 20, y2: 20 },
                    'R',
                );
                return [wireL, wireR];
            }
        ...
        switch (this.type) {
            ...
            case 'current-source-current-controlled': {
                const value = this.createTextElement(
                    { x: 78, y: 12 },
                    `1${defaultValue[this.type]}`,
                );
                value.setAttribute('class', 'component__text_value');
                this.value = 1;
                const sense = this.createTextElement(
                    { x: 70, y: 2 },
                    `SELECT YOUR SENSE`,
                );
    ...
    setOptionsText(option, initial = false) {
        switch (option) {
            case 'ac': {
                ...
                const magnitude = this.createTextElement(
                    { x: 70, y: 2 },
            ...
            case 'sine': {
                ...
                const frequency = this.createTextElement(
                    { x: 70, y: 32 },
            ...
            case 'unit': {
                ...
                amplitude.setAttribute('text-anchor', 'start');
                amplitude.setAttribute('class', 'component__option_amplitude');
                const trise = this.createTextElement(
                    { x: 70, y: 32 },
                    `trise: ${this.options.trise}`,
                );
    ...
    static createTextElement(position, content) {
    ...
    rotateComponent() {
        ...
        const transformInfo = this.svgElement
            .getAttribute('transform')
            .replace(
                /rotate\([^)]+\)/,
                `rotate(${this.rotation}, ${bbox.x + bbox.width / 2}, ${bbox.y + bbox.height / 2})`,
            );
        ...
        texts.forEach((text) => {
            const textTransformInfo = (
                text.getAttribute('transform') || ''
            ).replace(/\s*rotate\([^)]+\)\s*/, ' ');
            text.setAttribute(
                'transform',
                `${textTransformInfo} ${textRotateInfo}`,
            );
        });

        const wireS = Object.values(circuitWires).filter(
            (circuitWire) => circuitWire.start === this.num,
        );
        const wireE = Object.values(circuitWires).filter(
            (circuitWire) => circuitWire.end === this.num,
        );
    ...
    getLinePoint() {
        ...
        ['L', 'R', 'T', 'B', 'I', 'M'].forEach((direction) => {
            point[`point${direction}`] = this.calculatePoint(
                direction,
                lineInfo[direction],
            );
        });
    ...
    moveConnectedWires(wireS, wireE) {
        const { pointL, pointR, pointT, pointB, pointI, pointM } =
            this.getLinePoint();
    ...
    diverseComponent() {
        ...
        const transformInfo = this.svgElement
            .getAttribute('transform')
            .replace(
                /translate\([^)]*\)\s*scale\([^)]*\)\s*translate\([^)]*\)/,
                `translate(${cx}, ${cy}) scale(${this.diverse}, 1) translate(${-cx}, ${-cy})`,
            );
        this.svgElement.setAttribute('transform', transformInfo);
        ...
        texts.forEach((text) => {
            const textTransformInfo = (
                ...
                /\s*translate\([^)]*\)\s*scale\([^)]*\)\s*translate\([^)]*\)\s*/,
                ' ',
            );
            text.setAttribute(
                'transform',
                `${textTransformInfo} ${diverseInfo}`,
            );
        });
    ...
    removeConnection(pos, comp) {
        this.connections[`${this.num}${pos}`] = this.connections[
            `${this.num}${pos}`
        ].filter((component) => component !== comp);
        this.makeAPI('PUT');
    }
