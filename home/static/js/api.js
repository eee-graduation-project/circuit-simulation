...
const sendComponent = async (component, method) => {
    ...
    switch (method) {
        ...
        case 'PUT':
            await requestAPI(
                'PUT',
                `/api/component/${data.num}/?${new URLSearchParams({ boardId: data.boardId }).toString()}`,
                data,
            );
            break;
        case 'DELETE':
            await requestAPI(
                'DELETE',
                `/api/component/${component.num}/?${new URLSearchParams({ boardId: data.boardId }).toString()}`,
            );
            break;
        default:
            break;
...
const sendWire = async (wire, method) => {
    ...
    switch (method) {
        ...
        case 'PUT':
            await requestAPI(
                'PUT',
                `/api/wire/${data.num}/?${new URLSearchParams({ boardId: data.boardId }).toString()}`,
                data,
            );
            break;
        case 'DELETE':
            await requestAPI(
                'DELETE',
                `/api/wire/${wire.num}/?${new URLSearchParams({ boardId: data.boardId }).toString()}`,
            );
            break;
        default:
            break;
...
export const requestAPI = async (method, url, data) => {
    ...
    if (!response.ok) {
        throw new Error(responseData.simulation_error.message);
    }
    return responseData;
};
    ...
    modeFieldSets.forEach((fieldset) => {
        if (!fieldset.disabled)
            Array.from(fieldset.elements).forEach((input) => {
                fieldsetElement[input.name] = input.value;
            });
    });
...
const addProbe = async () => {
    ...
    return {
        ...
        probeVoltageMinus,
        probeCurrent,
        probeVoutPlus: probeVout.plus,
        probeVoutMinus: probeVout.minus,
    };
};
