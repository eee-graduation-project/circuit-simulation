...
const voltagePlusButton = document.querySelector(
    'img[alt="voltage-circle-plus"]',
);
const voltageMinusButton = document.querySelector(
    'img[alt="voltage-circle-minus"]',
);
...
voltagePlusButton.addEventListener('click', () => {
    addProbe('probe_voltage_plus');
});
voltageMinusButton.addEventListener('click', () => {
...
voutPlusButton.addEventListener('click', () => {
    addProbe('probe_vout_plus');
});
voutMinusButton.addEventListener('click', () => {
...
export const setProbe = (event, className, probeType) => {
    if (board.classList.contains(className)) {
        if (
            (probeNum.voutPlus && probeType === 'voutPlus') ||
            (probeNum.voutMinus && probeType === 'voutMinus')
        )
            return;
