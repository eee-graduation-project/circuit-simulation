...
class CustomJSONEncoder(json.JSONEncoder):  # by GPT
    def default(self, obj):
        if isinstance(obj, Symbol):
            ...
            return {
                "type": "Mul",
                "args": [self.default(arg) for arg in obj.args],
            }  # Mul 타입 처리
...
def simulate_circuit(request):
    ...
    if not board_id:
        return Response(
            {"error": "boardId is required"}, status=status.HTTP_400_BAD_REQUEST
        )

    [netlist, com2node, wire2node] = generate_netlist(board_id)
    [probeVoltage, probeCurrent, probeVout] = generate_probe(
        probes, com2node, wire2node
    )
    ...
    response = {
        ...
        "netlist": netlist,
        "analysis": analysis,
        "probeVoltage": probeVoltage,
        "probeCurrent": probeCurrent,
        "probeVout": probeVout,
    }
    try:
        [analysis_type, result] = cmd_analysis(
            netlist, analysis, probeCurrent, probeVoltage, probeVout
        )
